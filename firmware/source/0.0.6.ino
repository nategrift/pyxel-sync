...
bool sendHttpsRequest()
{
  ...
  if (httpCode > 0)
  {
    ...
    if (httpCode == HTTP_CODE_OK)
    {
      ...
      // Use the data
      bool success = applyAnimationData(responseBody, len);
    }
    else
    {
...
bool applyAnimationData(uint8_t *data, size_t dataSize)
{
  ...
  for (size_t frameIndex = 0; frameIndex < numFrames; ++frameIndex)
  {
    ...
    // Extract RGB values for each LED
    for (size_t ledIndex = 0; ledIndex < NUM_LEDS; ++ledIndex)
    {
      size_t pixelOffset = offset + ledIndex * 3;
      leds[ledIndex] = CRGB(data[pixelOffset], data[pixelOffset + 1], data[pixelOffset + 2]);
    }

    // Extract the frame duration in big-endian format
    unsigned long frameDuration = (unsigned long)data[offset + pixelsDataSize] << 24 |
    ...
    // Update the LEDs and wait for the frame duration
    FastLED.show();
    delay(frameDuration);
  }
